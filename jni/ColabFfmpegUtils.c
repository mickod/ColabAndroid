...
#define AUTO_INSERT_FILTER(opt_name, filter_name, arg) do {                 \
    AVFilterContext *filt_ctx;                                              \
                                                                            \
    av_log_Wrapper(NULL, AV_LOG_INFO, opt_name " is forwarded to lavfi "            \
...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Only video and audio filters supported "
               "currently.\n");
        exit_program(1);
    }
    ...
    if (in->name) {
        ...
        if (file_idx < 0 || file_idx >= nb_input_files) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid file index %d in filtergraph description %s.\n",
        ...
        if (!st) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Stream specifier '%s' in filtergraph description %s "
                   "matches no streams.\n", p, fg->graph_desc);
            exit_program(1);
        }
        ...
        if (i == nb_input_streams) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Cannot find a matching stream for "
...
static int configure_output_audio_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    if (audio_sync_method > 0) {
        ...
        av_log_Wrapper(NULL, AV_LOG_WARNING, "-async has been deprecated. Used the "
...
static void assert_codec_experimental(AVCodecContext *c, int encoder)
{
    ...
    if (c->codec->capabilities & CODEC_CAP_EXPERIMENTAL &&
        ...
        av_log_Wrapper(NULL, AV_LOG_FATAL, "%s '%s' is experimental and might produce bad "
                "results.\nAdd '-strict experimental' if you want to use it.\n",
                codec_string, c->codec->name);
        codec = encoder ? avcodec_find_encoder(c->codec->id) : avcodec_find_decoder(c->codec->id);
        if (!(codec->capabilities & CODEC_CAP_EXPERIMENTAL))
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Or use the non experimental %s '%s'.\n",
    ...
    if (codec && codec->sample_fmts) {
        ...
        if (*p == -1) {
            if((codec->capabilities & CODEC_CAP_LOSSLESS) && av_get_sample_fmt_name(st->codec->sample_fmt) > av_get_sample_fmt_name(codec->sample_fmts[0]))
                av_log_Wrapper(NULL, AV_LOG_ERROR, "Conversion will not be lossless.\n");
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO && pkt->dts != AV_NOPTS_VALUE) {
        ...
        if (ost->st->cur_dts && ost->st->cur_dts != AV_NOPTS_VALUE &&  max > pkt->dts) {
            av_log_Wrapper(s, max - pkt->dts > 2 ? AV_LOG_WARNING : AV_LOG_DEBUG, "Audio timestamp %"PRId64" < %"PRId64" invalid, cliping\n", pkt->dts, max);
            pkt->pts = pkt->dts = max;
        }
    }
...
static void pre_process_video_frame(InputStream *ist, AVPicture *picture, void **bufp)
{
    ...
    if (do_deinterlace) {
        ...
        if (avpicture_deinterlace(picture2, picture,
            ...
            av_log_Wrapper(NULL, AV_LOG_WARNING, "Deinterlacing failed\n");
            av_free(buf);
            buf = NULL;
            picture2 = picture;
        }
                         ...
                         float quality)
{
    ...
    if (nb_frames == 0) {
        nb_frames_drop++;
        av_log_Wrapper(NULL, AV_LOG_VERBOSE, "*** drop!\n");
        return;
    } else if (nb_frames > 1) {
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    av_log_Wrapper(NULL, AV_LOG_INFO, "%s    \r", buf);
    ...
    if (is_last_report) {
        ...
        av_log_Wrapper(NULL, AV_LOG_INFO, "video:%1.0fkB audio:%1.0fkB global headers:%1.0fkB muxing overhead %f%%\n",
               video_size / 1024.0,
               audio_size / 1024.0,
               extra_size / 1024.0,
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    for (i = 0; i < ist->nb_filters; i++) {
        ...
        if(av_buffersrc_add_frame(ist->filters[i]->filter, decoded_frame, 0)<0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Failed to inject frame into filter network\n");
            exit_program(1);
        }
 ...
 dump_format:
        ...
        for (j = 0; j < ist->nb_filters; j++) {
            ...
            if (ist->filters[j]->graph->graph_desc) {
                ...
                if (link->dst->input_count > 1)
                    av_log_Wrapper(NULL, AV_LOG_INFO, ":%s", link->dstpad->name);
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->filter && ost->filter->graph->graph_desc) {
            ...
            av_log_Wrapper(NULL, AV_LOG_INFO, "  %s", link->src->filter->name);
            if (link->src->output_count > 1)
                av_log_Wrapper(NULL, AV_LOG_INFO, ":%s", link->srcpad->name);
            if (nb_filtergraphs > 1)
        ...
        if (ost->stream_copy)
            ...
            av_log_Wrapper(NULL, AV_LOG_INFO, " (%s -> %s)", input_streams[ost->source_index]->dec ?
...
static int transcode(void)
{
    ...
    for (; received_sigterm == 0;) {
        ...
        if (output_packet(ist, &pkt) < 0 ||
            ...
            av_log_Wrapper(NULL, AV_LOG_ERROR, "Error while decoding stream #%d:%d\n",
                   ist->file_index, ist->st->index);
            if (exit_on_error)
                exit_program(1);
            av_free_packet(&pkt);
...
{
    av_log_Wrapper(NULL, AV_LOG_FATAL, "Option '%s' has been removed, use the crop filter instead\n", opt);
    return AVERROR(EINVAL);
}

static int opt_pad(const char *opt, const char *arg)
{
    av_log_Wrapper(NULL, AV_LOG_FATAL, "Option '%s' has been removed, use the pad filter instead\n", opt);
    return -1;
}

static int opt_video_channel(const char *opt, const char *arg)
{
    av_log_Wrapper(NULL, AV_LOG_WARNING, "This option is deprecated, use -channel.\n");
    return opt_default("channel", arg);
}

static int opt_video_standard(const char *opt, const char *arg)
{
    av_log_Wrapper(NULL, AV_LOG_WARNING, "This option is deprecated, use -standard.\n");
...
static int opt_map(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    if (sync = strchr(map, ',')) {
        *sync = 0;
        ...
        if (sync_file_idx >= nb_input_files || sync_file_idx < 0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid sync file index: %d.\n", sync_file_idx);
            exit_program(1);
        }
        if (*sync)
            sync++;
        ...
        if (i == input_files[sync_file_idx]->nb_streams) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Sync stream specification in map %s does not "
                                       "match any streams.\n", arg);
            exit_program(1);
        }
    }
    ...
    if (map[0] == '[') {
        ...
        if (!m->linklabel) {
            av_log_Wrapper(NULL, AV_LOG_ERROR, "Invalid output link label: %s.\n", map);
            exit_program(1);
        }
    } else {
        file_idx = strtol(map, &p, 0);
        if (file_idx >= nb_input_files || file_idx < 0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid input file index: %d.\n", file_idx);
            exit_program(1);
        }
        if (negative)
    ...
    if (!m) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Stream map '%s' matches no streams.\n", arg);
        exit_program(1);
    }
...
static int opt_map_channel(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    if (n != 3 && n != 5) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Syntax error, mapchan usage: "
    ...
    if (m->file_idx < 0 || m->file_idx >= nb_input_files) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "mapchan: invalid input file index: %d\n",
               m->file_idx);
        exit_program(1);
    }
    if (m->stream_idx < 0 ||
        m->stream_idx >= input_files[m->file_idx]->nb_streams) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "mapchan: invalid input file stream index #%d.%d\n",
               m->file_idx, m->stream_idx);
        exit_program(1);
    }
    st = input_files[m->file_idx]->ctx->streams[m->stream_idx];
    if (st->codec->codec_type != AVMEDIA_TYPE_AUDIO) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "mapchan: stream #%d.%d is not an audio stream.\n",
               m->file_idx, m->stream_idx);
        exit_program(1);
    }
    if (m->channel_idx < 0 || m->channel_idx >= st->codec->channels) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "mapchan: invalid audio channel #%d.%d.%d\n",
               m->file_idx, m->stream_idx, m->channel_idx);
        exit_program(1);
    }
    return 0;
...
static void parse_meta_type(char *arg, char *type, int *index, const char **stream_spec)
{
    if (*arg) {
        *type = *arg;
        ...
        case 's':
            if (*(++arg) && *arg != ':') {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid metadata specifier %s.\n", arg);
                exit_program(1);
            }
        ...
        case 'p':
            ...
            break;
        default:
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid metadata type %c.\n", *arg);
            exit_program(1);
        }
    } else
        *type = 'g';
...
#define METADATA_CHECK_INDEX(index, nb_elems, desc)\
    if ((index) < 0 || (index) >= (nb_elems)) {\
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid %s index %d while processing metadata maps.\n",\
                (desc), (index));\
        exit_program(1);\
    }
...
#define SET_DICT(type, meta, context, index)\
    ...
    if (type_in == 's') {
        ...
        if (!meta_in) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Stream specifier %s does not match  any streams.\n", istream_spec);
            exit_program(1);
        }
    }
...
static int opt_recording_timestamp(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    av_log_Wrapper(NULL, AV_LOG_WARNING, "%s is deprecated, set the 'creation_time' metadata "
                                 "tag instead.\n", opt);
    return 0;
}
...
static AVCodec *find_codec_or_die(const char *name, enum AVMediaType type, int encoder)
{
    ...
    if (!codec) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Unknown %s '%s'\n", codec_string, name);
        exit_program(1);
    }
    if (codec->type != type) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid %s type '%s'\n", codec_string, name);
        exit_program(1);
    }
    return codec;
}
    ...
    if ((!file_overwrite || no_file_overwrite) &&
        ...
        if (avio_check(filename, 0) == 0) {
            if (!using_stdin && (!no_file_overwrite || file_overwrite)) {
                ...
                if (!read_yesno()) {
                    av_log_Wrapper(NULL, AV_LOG_FATAL, "Not overwriting - exiting\n");
                    exit_program(1);
                }
                term_init();
            }
            else {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "File '%s' already exists. Exiting.\n", filename);
                exit_program(1);
            }
        }
    }
...
static void dump_attachment(AVStream *st, const char *filename)
{
    ...
    if (!st->codec->extradata_size) {
        av_log_Wrapper(NULL, AV_LOG_WARNING, "No extradata to dump in stream #%d:%d.\n",
    ...
    if (!*filename && (e = av_dict_get(st->metadata, "filename", NULL, 0)))
        filename = e->value;
    if (!*filename) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "No filename specified and no 'filename' tag"
               "in stream #%d:%d.\n", nb_input_files - 1, st->index);
        exit_program(1);
    }
    ...
    if ((ret = avio_open2(&out, filename, AVIO_FLAG_WRITE, &int_cb, NULL)) < 0) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not open file %s for writing.\n",
               filename);
        exit_program(1);
    }
...
static int opt_input_file(OptionsContext *o, const char *opt, const char *filename)
{
    ...
    if (o->format) {
        if (!(file_iformat = av_find_input_format(o->format))) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Unknown input format: '%s'\n", o->format);
            exit_program(1);
        }
    }
    ...
    if (ret < 0) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "%s: could not find codec parameters\n", filename);
    ...
    if (o->start_time != 0) {
        ...
        if (ret < 0) {
            av_log_Wrapper(NULL, AV_LOG_WARNING, "%s: could not seek to position %0.3f\n",
...
static uint8_t *get_line(AVIOContext *s)
{
    ...
    if (avio_open_dyn_buf(&line) < 0) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not alloc buffer for reading preset.\n");
        exit_program(1);
    }
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    if (!st) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not alloc stream.\n");
        exit_program(1);
    }
    ...
    if (preset && (!(ret = get_preset_file_2(preset, ost->enc->name, &s)))) {
        do  {
            ...
            if (!(arg = strchr(buf, '='))) {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid line found in the preset file.\n");
                exit_program(1);
            }
            *arg++ = 0;
    ...
    while (bsf) {
        ...
        if (!(bsfc = av_bitstream_filter_init(bsf))) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Unknown bitstream filter %s\n", bsf);
            exit_program(1);
        }
        if (bsfc_prev)
            bsfc_prev->next = bsfc;
...
static void parse_matrix_coeffs(uint16_t *dest, const char *str)
{
    ...
    for (i = 0;; i++) {
        ...
        p = strchr(p, ',');
        if (!p) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Syntax error in matrix \"%s\" at coeff %d\n", str, i);
            exit_program(1);
        }
        p++;
    }
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        ...
        if (frame_rate && av_parse_video_rate(&ost->frame_rate, frame_rate) < 0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid framerate value: %s\n", frame_rate);
            exit_program(1);
        }

        MATCH_PER_STREAM_OPT(frame_sizes, str, frame_size, oc, st);
        if (frame_size && av_parse_video_size(&video_enc->width, &video_enc->height, frame_size) < 0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid frame size: %s.\n", frame_size);
            exit_program(1);
        }
        ...
        if (frame_aspect_ratio) {
            ...
            if (av_parse_ratio(&q, frame_aspect_ratio, 255, 0, NULL) < 0 ||
                q.num <= 0 || q.den <= 0) {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid aspect ratio: %s\n", frame_aspect_ratio);
                exit_program(1);
            }
        ...
        if (frame_pix_fmt && (video_enc->pix_fmt = av_get_pix_fmt(frame_pix_fmt)) == PIX_FMT_NONE) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Unknown pixel format requested: %s.\n", frame_pix_fmt);
            exit_program(1);
        }
        ...
        if (intra_matrix) {
            if (!(video_enc->intra_matrix = av_mallocz(sizeof(*video_enc->intra_matrix) * 64))) {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not allocate memory for intra matrix.\n");
                exit_program(1);
            }
        ...
        if (inter_matrix) {
            if (!(video_enc->inter_matrix = av_mallocz(sizeof(*video_enc->inter_matrix) * 64))) {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not allocate memory for inter matrix.\n");
                exit_program(1);
            }
        ...
        for (i = 0; p; i++) {
            ...
            if (e != 3) {
                av_log_Wrapper(NULL, AV_LOG_FATAL, "error parsing rc_override\n");
                exit_program(1);
            }
...
static OutputStream *new_audio_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        ...
        if (sample_fmt &&
            ...
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid sample format '%s'\n", sample_fmt);
            exit_program(1);
        }
        ...
        for (n = 0; n < o->nb_audio_channel_maps; n++) {
            ...
            if ((map->channel_idx == -1 || (ist->file_index == map->file_idx && ist->st->index == map->stream_idx)) &&
                ...
                if (ost->audio_channels_mapped < FF_ARRAY_ELEMS(ost->audio_channels_map))
                    ...
                    av_log_Wrapper(NULL, AV_LOG_FATAL, "Max channel mapping for output %d.%d reached\n",
...
static OutputStream *new_data_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Data stream encoding not supported yet (only streamcopy)\n");
        exit_program(1);
    }
                               ...
                               AVFormatContext *oc)
{
    ...
    default:
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Only video and audio filters are supported "
               "currently.\n");
        exit_program(1);
    }
    ...
    if (ost->stream_copy) {
        av_log_Wrapper(NULL, AV_LOG_ERROR, "Streamcopy requested for output stream %d:%d, "
    ...
    if (configure_output_filter(ofilter->graph, ofilter, ofilter->out_tmp) < 0) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Error configuring filter.\n");
        exit_program(1);
    }
...
static void opt_output_file(void *optctx, const char *filename)
{
    ...
    if (configure_complex_filters() < 0) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Error configuring filters.\n");
        exit_program(1);
    }
    ...
    if (!strcmp(file_oformat->name, "ffm") &&
        ...
        for(j = nb_output_streams - oc->nb_streams; j < nb_output_streams; j++) {
            ...
            if(!ost->sync_ist){
                av_log_Wrapper(NULL, AV_LOG_FATAL, "Missing %s stream which is required by this ffm\n", av_get_media_type_string(ost->st->codec->codec_type));
                exit_program(1);
            }
        }
...
loop_end:
                if (!ofilter) {
                    av_log_Wrapper(NULL, AV_LOG_FATAL, "Output with label '%s' does not exist "
                ...
                default:
                    av_log_Wrapper(NULL, AV_LOG_FATAL, "Cannot map stream #%d:%d - unsupported type.\n",
    ...
    for (i = 0; i < o->nb_attachments; i++) {
        ...
        if ((err = avio_open2(&pb, o->attachments[i], AVIO_FLAG_READ, &int_cb, NULL)) < 0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not open attachment file %s.\n",
                   o->attachments[i]);
            exit_program(1);
        }
        if ((len = avio_size(pb)) <= 0) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not get size of the attachment %s.\n",
                   o->attachments[i]);
            exit_program(1);
        }
        if (!(attachment = av_malloc(len))) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Attachment %s too large to fit into memory.\n",
                   o->attachments[i]);
            exit_program(1);
        }
    ...
    for (i = 0; i < o->nb_metadata_map; i++) {
        ...
        if (in_file_index >= nb_input_files) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid input file index %d while processing metadata maps\n", in_file_index);
            exit_program(1);
        }
    ...
    if (o->chapters_input_file >= nb_input_files) {
        if (o->chapters_input_file == INT_MAX) {
            ...
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid input file index %d in chapter mapping.\n",
    ...
    for (i = 0; i < o->nb_metadata; i++) {
        ...
        if (!val) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "No '=' character in metadata string %s.\n",
                   o->metadata[i].u.str);
            exit_program(1);
        }
        *val++ = 0;
        ...
        if (type == 's') {
            ...
            case 'c':
                if (index < 0 || index >= oc->nb_chapters) {
                    av_log_Wrapper(NULL, AV_LOG_FATAL, "Invalid chapter index %d in metadata specifier.\n", index);
                    exit_program(1);
                }
...
{
    av_log_Wrapper(NULL, AV_LOG_INFO, "Hyper fast Audio and Video encoder\n");
    av_log_Wrapper(NULL, AV_LOG_INFO, "usage: %s [options] [[infile options] -i infile]... {[outfile options] outfile}...\n", program_name);
    av_log_Wrapper(NULL, AV_LOG_INFO, "\n");
}
...
static int opt_target(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    if (!strncmp(arg, "pal-", 4)) {
        ...
        if (norm != UNKNOWN)
            av_log_Wrapper(NULL, AV_LOG_INFO, "Assuming %s for target.\n", norm == PAL ? "PAL" : "NTSC");
    }

    if (norm == UNKNOWN) {
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Could not determine norm (PAL/NTSC/NTSC-Film) for target.\n");
        av_log_Wrapper(NULL, AV_LOG_FATAL, "Please prefix target with \"pal-\", \"ntsc-\" or \"film-\",\n");
        av_log_Wrapper(NULL, AV_LOG_FATAL, "or set a framerate with \"-r xxx\".\n");
        exit_program(1);
    }
    ...
    if (!strcmp(arg, "vcd")) {
        ...
        av_log_Wrapper(NULL, AV_LOG_ERROR, "Unknown target: %s\n", arg);
        return AVERROR(EINVAL);
    }
    return 0;
}
...
static int opt_preset(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    if (!(f = get_preset_file(filename, sizeof(filename), arg, *opt == 'f', codec_name))) {
        if(!strncmp(arg, "libx264-lossless", strlen("libx264-lossless"))){
            av_log_Wrapper(NULL, AV_LOG_FATAL, "Please use -preset <speed> -qp 0\n");
        }else
            av_log_Wrapper(NULL, AV_LOG_FATAL, "File for preset '%s' not found\n", arg);
        exit_program(1);
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!av_strtok(key,   "=",    &value) ||
            !av_strtok(value, "\r\n", &endptr)) {
            av_log_Wrapper(NULL, AV_LOG_FATAL, "%s: Invalid syntax: '%s'\n", filename, line);
            exit_program(1);
        }
        av_log_Wrapper(NULL, AV_LOG_DEBUG, "ffpreset[%s]: set '%s' = '%s'\n", filename, key, value);
        ...
        if      (!strcmp(key, "acodec")) opt_audio_codec   (o, key, value);
            ...
            av_log_Wrapper(NULL, AV_LOG_FATAL, "%s: Invalid option or argument: '%s', parsed as '%s' = '%s'\n",
...
{
    if(!strcmp(opt, "b")){
        av_log_Wrapper(NULL, AV_LOG_WARNING, "Please use -b:a or -b:v, -b is ambiguous\n");
...
static int opt_qscale(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "qscale")){
        av_log_Wrapper(NULL, AV_LOG_WARNING, "Please use -q:a or -q:v, -qscale is ambiguous\n");
...
static int opt_profile(OptionsContext *o, const char *opt, const char *arg)
{
    if(!strcmp(opt, "profile")){
        av_log_Wrapper(NULL, AV_LOG_WARNING, "Please use -profile:a or -profile:v, -profile is ambiguous\n");
...
{
    av_log_Wrapper(NULL, AV_LOG_WARNING, "-%s is deprecated, use -filter:v yadif instead\n", opt);
    do_deinterlace = 1;
    return 0;
}
...
static int opt_channel_layout(OptionsContext *o, const char *opt, const char *arg)
{
    ...
    if (!layout) {
        av_log_Wrapper(NULL, AV_LOG_ERROR, "Unknown channel layout: %s\n", arg);
        return AVERROR(EINVAL);
    }
...
JNIEXPORT jint JNICALL Java_com_amodtech_colabandroid_CompressionActivity_ffmpegWrapper(JNIEnv *pEnv, jobject pObj, jobjectArray javaArgv) {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "JNI ffmpegWrapper - updated argc: %d", argc);
    for ( i = 0; i < argc; ++i ) {
       __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "JNI ffmpegWrapper - updated argv[%d]: %s", i, argv[i]);
    }
    ...
    /* parse options */
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "JNI ffmpegWrapper about to parse options");
    parse_options(&o, argc, argv, options, opt_output_file);

    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Use -h to get full help or, even better, run 'man %s'\n", program_name);
    ...
    if (nb_output_files <= 0) {
    	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "At least one output file must be specified\n");
        av_log_Wrapper(NULL, AV_LOG_FATAL, "At least one output file must be specified\n");
        exit_program(1);
    }

    if (nb_input_files == 0) {
    	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "At least one input file must be specified\n");
    ...
    // cleanup the JNI argv alloctaion - not strictly speaking necessary as we call exit so the OS will clear any
    //memory allocated (apparently...)
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Freeing up JNI argv memory");
